...
- **DRAM** (Dynamic RAM) – paměťová buňka tvořena kondenzátorem a tranzistorem MOSFET. Je pomalejší (kvůli refreshi a destruktivnímu čtení), ale menší velikost buňky → větší kapacities. Využití: operační paměť, videopaměť.
...
- USB 1.1 (resp. USB 1.0): 12 Mb/s (resp. 1,5 Mb/s)  
- USB 2.0 (High-speed): 480 Mb/s  
- USB 3.0 (Super-speed): 4,7 Gb/s (600 MB/s)  
- USB 3.1 Gen 1: max. 5 Gb/s  
- USB 3.1 Gen 2: 10 Gb/s  

Jednotlivé standardy jsou vzájemně kompatibilní (vyjma kabeláže a konektorů u USB 3.0). Garantovaná komunikační vzdálenost do 5 m, u USB 3.0 se doporučuje max. 2,7 m. Možnost připojit až 127 zařízení v 7 úrovních. Napájecí napětí 5 V, max. proudový odběr 100 mA (resp. 500 mA u USB 2.0, 900 mA u USB 3.0). Podpora Plug and Play a Hot Plug. Přenos dat prostřednictvím USB rozhraní je řízen procesorem počítače.

*(Zdroj: Rozhrani.txt)*

### Otázka 50: Co je USB Type-C?  
Za označením USB-C (USB typu C) se skrývají pouze fyzické charakteristiky konektoru. Má 24 pinů, tvar podobný microUSB a možnost převrátit jej při zapojování (symetrický konektor). Za USB-C se může skrývat USB 3.1, Thunderbolt 3, ale i starší USB 3.0 nebo dokonce USB 2.0. USB4 (představené 2019, založené na Thunderbolt 3) nabízí propustnost 20 Gb/s (Gen 2×2), resp. 40 Gb/s (Gen 3×2). USB4 Version 2.0 podporuje až 80 Gb/s, avšak vyžaduje nové aktivní kabely USB-C.

*(Zdroj: Rozhrani.txt)*

### Otázka 51: Jaké jsou parametry Bluetooth?  
Bluetooth je bezdrátové komunikační rozhraní definované standardem IEEE 802.15 (PAN – Personal Area Network). Pracuje v nelicencovaném pásmu 2,4 GHz (2,4–2,483 GHz), rozděleném na 79 kanálů (šířka 1 MHz). Provádí 1600 přeskoků frekvence za sekundu. Před prvním spojením je nutné spárování pomocí PIN. Tři třídy zařízení podle vysílacího výkonu: Class 1 (10 mW, dosah cca 100 m), Class 2 (2,5 mW, dosah cca 50 m), Class 3 (1 mW, dosah cca 10 m). Verze: BT 2.0/2.1 EDR (až 3 Mb/s, snížené nároky na napájení), BT 3.0 HS (až 24 Mb/s, založen na protokolu Wi-Fi 802.11, vylepšené zabezpečení).

*(Zdroj: Rozhrani.txt)*

### Otázka 52: Co je Thunderbolt 3?  
Thunderbolt 3 používá konektor USB-C a podporuje protokoly USB 3.1, USB 3.0 a starší, DisplayPort 1.2 i originální Thunderbolt. Vše běží přes interní sběrnici PCI Express 3.0. Nabízí dvojnásobnou propustnost oproti druhé generaci – až 40 Gb/s přes jediný aktivní měděný kabel. Thunderbolt 4 pak garantuje podporu dvou 4K nebo jednoho 8K displeje, PCIe propustnost 32 Gb/s, napájení až 100 W a ochranu přes Intel VT-d proti zneužití přímého přístupu do paměti (DMA).
